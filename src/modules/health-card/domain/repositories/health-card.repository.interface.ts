...
export interface IHealthCardRepository {
  findProfileByUserId(userId: string): Promise<{ id: string; firstName: string; lastName: string } | null>;
  ...
  findQrByPayload(encryptedPayload: string): Promise<(HealthCardQrEntity & { healthCard: HealthCardEntity }) | null>;
